...
var resizePizzas = function (size) {
  ...
  // 遍历披萨的元素并改变它们的宽度，
  //这里主要省去大部分过程，因为只需要改变图片的大小，所以根据不同size设置不同的百分比就可实现。
  /**
   * @param {string} size 
   */
  function changePizzaSizes(size) {
    let newwidth;
    let newwidthArr =  new Int16Array([25,33,50]);
    newwidth =  newwidthArr[Number(size)-1]
    let randomPizza = document.querySelectorAll(".randomPizzaContainer")
    for (let i = 0; i < randomPizza.length; i++) {
...
function updatePositions() {
  ...
  let top = document.body.scrollTop / 1250;
  for (var i = 0; i < items.length; i++) {
    // 既然要取余，在DOMContentLoaded时候计算一次，
     //V8访问隐藏类的属性数组的速度应该会快于计算
    var phase = Math.sin(top + items[i].mod); 
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // updatePositions();将此函数改为requestAnimationFrame
